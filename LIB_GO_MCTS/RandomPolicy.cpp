...
//}
//==================================================================================
short RandomPolicy::GeneratePlayOutMove(SimpleGoGame& go_game_ref)
{
    //Generate all legal moves
    std::vector<short> legal_moves;
    ...
    while(!legal_moves.empty())
    {
        ...
        if( GobanUtils::IsOnePointEye(board_pos.first, board_pos.second, go_game_ref.GetMainBoard(), go_game_ref.GetCurrentPlayer()) )
        {
            // remove it from legal moves and choose another one
            legal_moves.erase(legal_moves.begin() + position);
            continue;

        }else if(GobanUtils::IsPotentialOnePointEye(board_pos.first, board_pos.second, go_game_ref.GetMainBoard(), go_game_ref.GetCurrentPlayer()))
        {
            //delay it
            potential_eye_vec.push_back(move);
