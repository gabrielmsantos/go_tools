...
int BasicGobanStatistics::GetEstimativeTerritory(StoneState stone,const Goban& r_goban, I_InfluenceModel& r_influence)
{
    ...
    for(unsigned int i = 0; i< r_influence.GetDimension(); ++i)
        for(unsigned int j = 0; j< r_influence.GetDimension(); ++j)
    {
        if((stone > 0 ) && ((r_influence.GetInfluence(i,j) >= 13) || ((r_goban.GetLayer(i,j)<=2)&&(r_influence.GetInfluence(i,j) >= 6.5f))) && (r_goban.GetStone(i,j)!= stone))
        {
            total++;
        }
        else if ((stone < 0 ) && ((r_influence.GetInfluence(i,j) <= -13) || ((r_goban.GetLayer(i,j)<=2)&&(r_influence.GetInfluence(i,j) <= -6.5f)))  && (r_goban.GetStone(i,j)!= stone))
        {
            total++;
        }
    }
